// This file includes functions to compare the two models loaded from the files.
// It determines differences and similarities between the models.
 ...
 */
function getPolyElementKey(vertexCoordsList, floorId = '', sectionId = '', precision = PRECISION) {
 ...
 */
export function lineElementKeyExtractor(element, nodeMap, idStartAttr, idEndAttr) {
  ...
  console.warn(`Missing node coords for line element: Start=${startId}, End=${endId}, ElementID=${elementId}`);
  return { key: null, data: null };
}

/**
 * ポリゴン要素（スラブ、壁など）から比較キーと関連データ（頂点座標リスト）を抽出する。
 * @param {Element} element - ポリゴン要素のXML要素。
 * @param {Map<string, {x: number, y: number, z: number}>} nodeMap - 対応するノードマップ。
 * @param {string} [nodeOrderTag="StbNodeIdOrder"] - 頂点ノードIDリストが含まれるタグ名。
 * @returns {{key: string|null, data: {vertexCoordsList: Array<object>}|null}} キーとデータのオブジェクト。
 */
export function polyElementKeyExtractor(element, nodeMap, nodeOrderTag = "StbNodeIdOrder") {
  ...
  if (orderElem && orderElem.textContent) {
    ...
    if (vertexCoordsList.length === nodeIds.length && vertexCoordsList.length >= 3) {
      ...
      return { key, data: { vertexCoordsList } };
    } else {
      console.warn(`Missing node coords or insufficient vertices for poly element: IDs=${nodeIds}, Found=${vertexCoordsList.length}`);
    }
  }
  return { key: null, data: null };
}
...
// Additional utility functions for comparison can be added here.