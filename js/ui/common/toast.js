 ...
 * イベントバス経由でトースト通知を表示
 *
 * この関数はアプリケーション初期化時に呼び出され、
 * イベントバス経由でトースト通知を表示できるようにします。
 * 下位レイヤーがUI層に直接依存しないようにするための仕組みです。
 */
export function initializeToastEventListeners() {
  eventBus.on(ToastEvents.SHOW_ERROR, ({ message, options }) => {
    showError(message, options);
  });

  eventBus.on(ToastEvents.SHOW_WARNING, ({ message, options }) => {
    showWarning(message, options);
  });

  eventBus.on(ToastEvents.SHOW_SUCCESS, ({ message, options }) => {
    showSuccess(message, options);
  });

  eventBus.on(ToastEvents.SHOW_INFO, ({ message, options }) => {
