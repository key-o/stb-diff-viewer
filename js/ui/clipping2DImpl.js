...
import { getCameraMode } from '../viewer/camera/cameraManagerImpl.js';
...
/**
 * 2D奥行きクリッピングのUI状態を外部から更新（アダプタ用）
 * @param {boolean} enabled - クリッピングが有効かどうか
 */
export function updateDepth2DClippingUIInternal(enabled) {
  const applyButton = document.getElementById('applyDepth2DClipButton');
  const clippingGroup = document.getElementById('depth2DClippingGroup');
...
export function initDepth2DClippingUI() {
  ...
  if (rangeSlider && rangeValue) {
    rangeSlider.addEventListener('input', (e) => {
      ...
      // クリッピングが有効な場合は自動更新
      if (current2DClippingState.enabled) {
        applyDepth2DClipping(current2DClippingState.centerZ, current2DClippingState.range);
      }
    });
  }
...
eventBus.on(ModelEvents.BOUNDS_UPDATED, ({ modelBounds }) => {
