...
export const DIFF_STATUS = {
  // 存在差分
  MATCHED: 'matched', // 完全一致（位置・属性とも一致）
  ONLY_A: 'onlyA', // モデルAのみに存在
  ONLY_B: 'onlyB', // モデルBのみに存在
  // 内容差分（両方に存在する要素の詳細分類）
  POSITION_TOLERANCE: 'positionTolerance', // 位置が許容差内、属性一致
  ATTRIBUTE_MISMATCH: 'attributeMismatch', // 位置完全一致、属性不一致
  COMBINED: 'combined', // 位置が許容差内 + 属性不一致
  // レガシー互換性のためのエイリアス
  MISMATCH: 'mismatch', // 後方互換: attributeMismatchと同義
};
 ...
 * @constant {Object}
 */
export const DIFF_STATUS_NAMES = {
  [DIFF_STATUS.MATCHED]: '完全一致', // 緑色
  [DIFF_STATUS.ONLY_A]: 'モデルAのみ', // 青色
  [DIFF_STATUS.ONLY_B]: 'モデルBのみ', // 赤色
  [DIFF_STATUS.POSITION_TOLERANCE]: '位置許容差内', // 黄色
  [DIFF_STATUS.ATTRIBUTE_MISMATCH]: '属性不一致', // オレンジ色
  [DIFF_STATUS.COMBINED]: '複合差分', // 紫色
  // レガシー互換
  [DIFF_STATUS.MISMATCH]: '属性不一致', // オレンジ色（後方互換）
};

/**
 * 新しい6カテゴリステータス（レガシーmismatchを除く）
 * UI表示やフィルタリングで使用
 * @constant {Array<string>}
 */
export const DIFF_STATUS_VALUES = [
  DIFF_STATUS.MATCHED,
  DIFF_STATUS.ONLY_A,
  DIFF_STATUS.ONLY_B,
...
export class DiffStatusFilter {
  ...
  createDefaultPresets() {
    return {
      all: {
        name: '全て表示',
        levels: new Set(DIFF_STATUS_VALUES),
        description: 'すべての差分ステータスを表示',
      },
      differencesOnly: {
        name: '差分のみ',
        levels: new Set([DIFF_STATUS.ONLY_A, DIFF_STATUS.ONLY_B]),
        description: 'モデルAのみ・モデルBのみの要素を表示（追加・削除）',
      },
      matchedOnly: {
        name: '共通部材のみ',
        levels: new Set([
        ...
        description: '両方のモデルに存在する要素のみ表示',
      },
      onlyAOnly: {
        name: 'モデルAのみ',
        levels: new Set([DIFF_STATUS.ONLY_A]),
        description: 'モデルAにのみ存在する要素を表示',
      },
      onlyBOnly: {
        name: 'モデルBのみ',
        levels: new Set([DIFF_STATUS.ONLY_B]),
        description: 'モデルBにのみ存在する要素を表示',
      },
      changesOnly: {
        name: '変更・差分のみ',
        levels: new Set([
        ...
        description: '変更があった要素のみ表示（追加、削除、位置変更、属性変更）',
      },
      exactMatchOnly: {
        name: '完全一致のみ',
        levels: new Set([DIFF_STATUS.MATCHED]),
        description: '位置・属性とも完全に一致する要素のみ表示',
      },
    };
  }
  ...
  _getSingleModelStatus() {
    const modelBDocument = getState('models.modelBDocument');
    const modelADocument = getState('models.modelADocument');
