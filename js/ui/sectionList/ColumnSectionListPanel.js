...
import { extractColumnSectionList, extractColumnSectionGrid } from '../../data/extractors/columnSectionListExtractor.js';
...
export class ColumnSectionListPanel {
    ...
    setTimeout(() => {
      try {
        ...
        log.info('Section grid rendered', {
          ...
          totalCells: (this.currentData.stories?.length || 0) * (this.currentData.symbols?.length || 0),
        });
      } catch (error) {
  ...
  async handleExportPdf() {
    ...
    const table = tableContainer?.querySelector('.column-section-grid-table') ||
