...
export class BeamSectionListRenderer {
  ...
  renderSectionCell(sectionData) {
    ...
    parts.push(`<tr><td class="spec-label">コンクリート</td><td class="spec-value">${concreteStrength}</td></tr>`);

    // 上端筋（複数位置対応）
    const topBarText = this.formatMultiPositionBar(
      sectionData.positions,
      'topBar',
      positionKeys
    );
    parts.push(`<tr><td class="spec-label">上端筋</td><td class="spec-value">${topBarText}</td></tr>`);
    ...
    parts.push(`<tr><td class="spec-label">スターラップ</td><td class="spec-value">${stirrupText}</td></tr>`);

    // 腹筋（存在する場合のみ）
    const webBarText = this.formatMultiPositionBar(
      sectionData.positions,
      'webBar',
      positionKeys
    );
    if (webBarText !== '-' && webBarText.trim() !== '') {
      parts.push(`<tr><td class="spec-label">腹筋</td><td class="spec-value">${webBarText}</td></tr>`);
    }
