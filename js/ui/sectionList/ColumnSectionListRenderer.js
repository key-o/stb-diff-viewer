...
export class ColumnSectionListRenderer {
  ...
  renderSectionCell(sectionData) {
    ...
    parts.push(`<tr><td class="spec-label">B×D</td><td class="spec-value">${dimText} (${concreteStrength})</td></tr>`);

    // 主筋
    const mainBarText = this.formatMainBar(sectionData);
    parts.push(`<tr><td class="spec-label">主筋</td><td class="spec-value">${mainBarText}</td></tr>`);

    // 材料（主筋）
    if (sectionData.mainBar?.grade) {
      parts.push(`<tr><td class="spec-label">材料</td><td class="spec-value">${sectionData.mainBar.grade}</td></tr>`);
    }

    // 1段目dt
    const dtText = this.formatDt(sectionData);
    if (dtText) {
      parts.push(`<tr><td class="spec-label">1段目dt</td><td class="spec-value">${dtText}</td></tr>`);
    }
    ...
    if (this.options.showCoreBar && sectionData.coreBar) {
      ...
      if (coreBarText) {
        parts.push(`<tr><td class="spec-label">芯鉄筋</td><td class="spec-value">${coreBarText}</td></tr>`);
      }
    }
