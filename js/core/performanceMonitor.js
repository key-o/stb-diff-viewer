...
class PerformanceMonitor {
  ...
  recordRenderTime(renderTime) {
    ...
    if (renderTime > 16.67) {
      // 60FPS以下
      console.warn(
        `Low framerate detected: render took ${renderTime.toFixed(2)}ms`
      );
    }
  }
  ...
  getStats() {
    ...
    if (this.renderTimes.length > 0) {
      ...
      const totalTime = recentRenderTimes.reduce(
        (sum, item) => sum + item.time,
        0
      );

      stats.rendering = {
        averageFrameTime: parseFloat(
          (totalTime / recentRenderTimes.length).toFixed(2)
        ),
        estimatedFPS: parseFloat(
          (1000 / (totalTime / recentRenderTimes.length)).toFixed(1)
        ),
  ...
  logStats() {
    ...
    if (stats.rendering.frameCount > 0) {
      ...
      console.log(
        "Average frame time:",
        `${stats.rendering.averageFrameTime}ms`
      );
  ...
  decorateMethod(target, methodName, displayName = null) {
    ...
    const monitorName =
      displayName || `${target.constructor.name}.${methodName}`;

    target[methodName] = (...args) => {
      return this.measure(monitorName, () =>
 ...
 */
export const {
  setEnabled: enablePerformanceMonitoring,
