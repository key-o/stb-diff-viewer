...
class ComparisonKeyManager {
  constructor() {
    ...
    console.log(`[ComparisonKeyManager] Initialized with key type: ${this.currentKeyType}`);
  }

  /**
   * LocalStorageからキータイプを読み込む
   * @returns {string} 比較キータイプ
   * @private
   */
  loadFromStorage() {
    try {
      const saved = localStorage.getItem(COMPARISON_KEY_TYPE_STORAGE_KEY);
      if (saved && Object.values(COMPARISON_KEY_TYPE).includes(saved)) {
        console.log(`[ComparisonKeyManager] Loaded from storage: ${saved}`);
        return saved;
      }
    } catch (error) {
      console.warn('[ComparisonKeyManager] Failed to load from storage:', error);
    }
  ...
  /**
   * LocalStorageにキータイプを保存する
   * @param {string} keyType - 比較キータイプ
   * @private
   */
  saveToStorage(keyType) {
    try {
      localStorage.setItem(COMPARISON_KEY_TYPE_STORAGE_KEY, keyType);
      console.log(`[ComparisonKeyManager] Saved to storage: ${keyType}`);
    } catch (error) {
      console.error('[ComparisonKeyManager] Failed to save to storage:', error);
    }
  }
  ...
  setKeyType(keyType) {
    ...
    if (oldKeyType === keyType) {
      console.log('[ComparisonKeyManager] Key type already set');
      return true;
    }
    ...
    console.log(`[ComparisonKeyManager] Key type changed: ${oldKeyType} -> ${keyType}`);
    return true;
  }
