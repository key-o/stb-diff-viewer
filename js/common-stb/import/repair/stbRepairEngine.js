/**
 * @fileoverview ST-Bridgeデータ修復エンジン（オプション機能）
 *
 ...
 * デフォルトでは無効（何も注入されていない状態）であり、
 * 利用するアプリケーションが `setValidatorFunctions` で明示的に
 * バリデーション関数を注入した場合のみ動作します（オプトイン）。
 *
 * MatrixCalc の `common/stb/repair/stb-repair-engine.js` から
 * 統一計画フェーズ2で移植。SDV側は未配線。
 *
 * @module common-stb/import/repair/stbRepairEngine
 */
