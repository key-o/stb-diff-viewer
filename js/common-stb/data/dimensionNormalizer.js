 ...
 *
 * @module common/stb/data/dimensionNormalizer
 */
...
export function deriveDimensionsFromAttributes(attrMap) {
  ...
  for (const attr of attributes) {
    ...
    if (w === undefined) {
      if (WIDTH_KEYS.includes(name)) {
        ...
        w = num;
        originalAttrs.width_X = num; // 元の属性名を保持
      }
    }
    ...
    if (h === undefined) {
      if (HEIGHT_KEYS.includes(name)) {
        ...
        h = num;
        originalAttrs.width_Y = num; // 元の属性名を保持
      }
    }

    // depth属性の抽出（フーチング等で使用）
    if (name === 'depth' || name === 'Depth') {
      originalAttrs.depth = num;
    }
    ...
    if (
      t === undefined &&
      (name === 't' || name === 'thickness' || name === 't1' || name === 't_T')
    ) {
      t = num;
    }
  ...
  // 注: 壁などは厚さ(t)のみを持つ場合があるため、厚さもチェック
  if (
    w === undefined &&
    h === undefined &&
    t === undefined &&
    length_pile_local === undefined &&
    !hasExtendedPileAttrs
  ) {
    return null;
  }
  ...
  if (typeof D_local === 'number') {
    ...
    // D が存在すれば常に円形断面として扱う（Option A修正）
    // 明示的なwidth/heightがあっても、Dがあれば円形
    out.profile_hint = 'CIRCLE';
  }

  // 元の属性名を出力にマージ（Option A対応）
  // width_X, width_Y, depth などの元の属性名を保持
  if (Object.keys(originalAttrs).length > 0) {
    Object.assign(out, originalAttrs);
  }
...
export function isExtendedPile(dimensions) {
  ...
  // 拡底杭固有の属性が存在する場合
  // pile_type が 'Straight' の場合は拡張杭とみなさない
  return !!(
    dimensions.D_extended_foot ||
    dimensions.D_extended_top ||
    (dimensions.pile_type && dimensions.pile_type !== 'Straight')
  );
}
