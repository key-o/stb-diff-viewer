...
// バリデーション連携機能をインポート
import { generateValidationInfoHtml } from '../../../validation/validationIntegration.js';
 ...
 */
function renderJointParentInfo(jointMeshDataA, jointMeshDataB, showSingleColumn, modelSource) {
...
function showJointMeshDataOnly(panel, title, contentDiv, jointMeshDataA, jointMeshDataB) {
  ...
  content += '</tbody></table>';
  content += '<p style="color: orange; font-size: 0.9em; margin-top: 8px;">※ XML内に継手定義が見つかりませんでした。メッシュデータのみを表示しています。</p>';
...
export async function displayElementInfo(idA, idB, elementType, modelSource = null) {
  ...
  if (elementType === 'Joint') {
    ...
    const jointIdA =
      jointMeshDataA?.jointId?.toString() || extractJointIdFromMeshId(idA);
    const jointIdB =
      jointMeshDataB?.jointId?.toString() || extractJointIdFromMeshId(idB);

    logger.debug(`Joint ID extraction: idA=${idA} -> jointIdA=${jointIdA}, idB=${idB} -> jointIdB=${jointIdB}`);
  ...
  if (!nodeA && !nodeB) {
    ...
    if (elementType === 'Joint' && (jointMeshDataA || jointMeshDataB)) {
      logger.warn(
        `Joint XML not found for ID A:${idA} or B:${idB}, showing mesh data only.`,
      );
  ...
  const schemaElementName =
    actualElementType === 'Node' ? 'StbNode' : `Stb${actualElementType}`;
  ...
  if (nodeA && nodeB) {
    ...
    title = `比較: ${actualElementType}${parentInfo} (A: ${idADisplay}, B: ${idBDisplay})${typeNote}${schemaInfo}`;
  } else if (nodeA) {
    title = `モデル A: ${actualElementType}${parentInfo} (ID: ${displayId})${typeNote}${schemaInfo}`;
  } else {
    title = `モデル B: ${actualElementType}${parentInfo} (ID: ${displayId})${typeNote}${schemaInfo}`;
  }
