...
import { SECTION_TYPE } from './sectionTypeUtil.js';
import {
  getSectionType,
  getWidth,
  getHeight,
...
export function validateSectionType(sectionData) {
  ...
  if (!VALIDATION_CONFIG.allowedSectionTypes.includes(sectionType)) {
    ...
    if (VALIDATION_CONFIG.allowedSectionTypes.includes(normalized)) {
      warnings.push(
        `Section type "${sectionType}" should be normalized to "${normalized}"`
      );
    } else {
      warnings.push(
 ...
 * @returns {Object} バリデーション結果
 */
export function validateDimensions(sectionData, elementType) {
  const errors = [];
  const warnings = [];
  const values = {};
  ...
  if (isCircular) {
    ...
    if (thickness !== undefined) {
      ...
      if (diameter !== undefined && thickness >= diameter / 2) {
        errors.push(
          `Thickness (${thickness}mm) must be less than radius (${diameter / 2}mm)`
        );
      }
    }
  } else {
