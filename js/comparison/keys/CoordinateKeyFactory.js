/**
 * @fileoverview 比較キー生成ファクトリー
 *
 * モデル要素の比較用キー文字列を生成するためのファクトリーモジュール。
...
export function getNodeCoordKey(coords, precision = PRECISION) {
  if (
    ...
    console.warn('[CoordinateKeyFactory] キー生成: 無効な座標', coords);
    return null;
  }
...
export function getElementKey(element, keyType, positionKeyGenerator) {
  if (keyType === COMPARISON_KEY_TYPE.GUID_BASED) {
    ...
    console.warn(
      `[CoordinateKeyFactory] 比較キー: GUIDなし、位置ベースにフォールバック (id=${element.getAttribute('id')})`,
    );
  }
...
/**
 * CoordinateKeyFactoryクラス
 * キー生成のためのファクトリークラス（オブジェクト指向インターフェース）
 */
export class CoordinateKeyFactory {
  /**
   * @param {number} [precision=PRECISION] - 座標精度
   */
  ...
  /**
   * 節点座標キーを生成
   * @param {{x: number, y: number, z: number}} coords - 座標
   * @returns {string|null} キー文字列
   */
  createNodeKey(coords) {
    return getNodeCoordKey(coords, this.precision);
  }

  /**
   * 線分要素キーを生成
   * @param {{x: number, y: number, z: number}} startCoords - 始点
   * @param {{x: number, y: number, z: number}} endCoords - 終点
   * @returns {string|null} キー文字列
   */
  createLineKey(startCoords, endCoords) {
    return getLineElementKey(startCoords, endCoords, this.precision);
  }

  /**
   * ポリゴン要素キーを生成
   * @param {Array<{x: number, y: number, z: number}>} vertexCoordsList - 頂点リスト
   * @param {string} [floorId=''] - 床ID
   * @param {string} [sectionId=''] - 断面ID
   * @returns {string|null} キー文字列
   */
  createPolyKey(vertexCoordsList, floorId = '', sectionId = '') {
    return getPolyElementKey(vertexCoordsList, floorId, sectionId, this.precision);
  }

  /**
   * 要素キーを生成（キータイプ対応）
   * @param {Element} element - XML要素
   * @param {string} keyType - キータイプ
   * @param {function} positionKeyGenerator - 位置ベースキー生成関数
   * @returns {string|null} キー文字列
   */
  createElementKey(element, keyType, positionKeyGenerator) {
    return getElementKey(element, keyType, positionKeyGenerator);
  }

  /**
   * GUIDキーを生成
   * @param {Element} element - XML要素
   * @returns {string|null} GUIDキー
   */
  createGuidKey(element) {
    return getGuidKey(element);
  }
}

// デフォルトインスタンス
export const defaultKeyFactory = new CoordinateKeyFactory();
