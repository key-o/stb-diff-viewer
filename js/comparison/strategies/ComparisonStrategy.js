 ...
 * 基本比較戦略（厳密一致）
 */
export class BasicComparisonStrategy extends ComparisonStrategy {
  get name() {
    return 'basic';
  }
 ...
 * 利用可能な比較戦略を管理
 */
export class ComparisonStrategyRegistry {
  constructor() {
    this.strategies = new Map();
    this.defaultStrategy = new BasicComparisonStrategy();
  }

  /**
   * 戦略を登録
   * @param {string} name - 戦略名
   * @param {ComparisonStrategy} strategy - 戦略インスタンス
   */
  register(name, strategy) {
    if (!(strategy instanceof ComparisonStrategy)) {
      throw new Error('Strategy must extend ComparisonStrategy');
    }
   ...
   * 戦略を取得
   * @param {string} name - 戦略名
   * @returns {ComparisonStrategy} 戦略インスタンス
   */
  get(name) {
   ...
   * オプションに基づいて適切な戦略を選択
   * @param {ComparisonOptions} options - 比較オプション
   * @returns {ComparisonStrategy} 選択された戦略
   */
  selectStrategy(options = {}) {
    for (const strategy of this.strategies.values()) {
      if (strategy.isApplicable(options)) {
        return strategy;
      }
    }
...
// デフォルトレジストリ
export const defaultRegistry = new ComparisonStrategyRegistry();
defaultRegistry.register('basic', new BasicComparisonStrategy());
