/**
 * Type3: Attribute Addition/Removal
...
/**
 * Default values for new attributes in v2.1.0
 */
const V210_DEFAULTS = {
  StbStory: {
    kind: 'GENERAL',
    // level_name is generated from height
    // strength_concrete is optional
  },
  StbSlab: {
    kind_structure: 'RC',
    kind_slab: 'NORMAL',
...
/**
 * Default values for removed attributes when converting to v2.0.2
 */
const V202_DEFAULTS = {
  StbColumn: {
...
export function convertStoryAttributesTo210(stbRoot) {
  ...
  stories.forEach((story) => {
    ...
    if (!attrs['kind']) {
      attrs['kind'] = V210_DEFAULTS.StbStory.kind;
      count++;
    }
  });
...
/**
 * Attributes removed from StbColumn in v2.1.0
 */
const COLUMN_ATTRS_REMOVED_IN_210 = [
  'condition_bottom',
  'condition_top',
  'joint_top',
  'joint_bottom',
...
/**
 * Attributes removed from StbGirder/StbBeam in v2.1.0
 * - condition_* : Removed (boundary conditions)
 * - joint_* : Moved to StbJointArrangement elements
 * - haunch_* : Moved to StbCalGirder (calculation model)
 */
const GIRDER_ATTRS_REMOVED_IN_210 = [
  'condition_start',
  'condition_end',
  'kind_joint_start',
  'kind_joint_end',
...
/**
 * Attributes removed from StbBrace in v2.1.0
 * - condition_* : Removed (boundary conditions)
 * - offset_* : Removed (replaced by aim_offset_* in v2.1.0)
 */
const BRACE_ATTRS_REMOVED_IN_210 = [
  'condition_start',
  'condition_end',
...
/**
 * Convert StbColumn attributes from v2.0.2 to v2.1.0
 * @param {object} stbRoot - ST-Bridge root element
 */
export function convertColumnAttributesTo210(stbRoot) {
  const root = getStbRoot(stbRoot);
  if (!root) {
    logger.warn('convertColumnAttributesTo210: root not found');
    return;
  }
  const rootData = Array.isArray(root) ? root[0] : root;
  const members = rootData?.['StbModel']?.[0]?.['StbMembers']?.[0];
  if (!members) {
    logger.warn('convertColumnAttributesTo210: members not found');
    return;
  }
  ...
  if (columns) {
    columns.forEach((column) => {
      ...
      COLUMN_ATTRS_REMOVED_IN_210.forEach((key) => {
  ...
  // Process StbPosts (same attributes as columns)
  ...
  if (count > 0) {
    logger.info(`StbColumn/StbPost: Removed ${count} v2.0.2 attributes`);
  } else {
    logger.debug('convertColumnAttributesTo210: no attributes removed');
  }
}

/**
 * Convert StbGirder/StbBeam attributes from v2.0.2 to v2.1.0
 * @param {object} stbRoot - ST-Bridge root element
 */
export function convertGirderBeamAttributesTo210(stbRoot) {
  const root = getStbRoot(stbRoot);
  if (!root) return;
  const rootData = Array.isArray(root) ? root[0] : root;
  const members = rootData?.['StbModel']?.[0]?.['StbMembers']?.[0];
  ...
  if (girders) {
    girders.forEach((girder) => {
      ...
      GIRDER_ATTRS_REMOVED_IN_210.forEach((key) => {
        if (attrs[key] !== undefined) {
          delete attrs[key];
          count++;
        }
  ...
  if (count > 0) {
    logger.info(`StbGirder/StbBeam: Removed ${count} v2.0.2 attributes`);
  }
}

/**
 * Convert StbBrace attributes from v2.0.2 to v2.1.0
 * @param {object} stbRoot - ST-Bridge root element
 */
export function convertBraceAttributesTo210(stbRoot) {
  const root = getStbRoot(stbRoot);
  if (!root) return;
  const rootData = Array.isArray(root) ? root[0] : root;
  const braces = rootData?.['StbModel']?.[0]?.['StbMembers']?.[0]?.['StbBraces']?.[0]?.['StbBrace'];
  if (!braces) return;
  ...
  braces.forEach((brace) => {
    ...
    BRACE_ATTRS_REMOVED_IN_210.forEach((key) => {
      if (attrs[key] !== undefined) {
        delete attrs[key];
        count++;
      }
  ...
  if (count > 0) {
    logger.info(`StbBrace: Removed ${count} v2.0.2 attributes`);
  }
}

/**
 * Convert StbColumn attributes from v2.1.0 to v2.0.2
 * @param {object} stbRoot - ST-Bridge root element
 */
export function convertColumnAttributesTo202(stbRoot) {
  const root = getStbRoot(stbRoot);
  const columns =
    root?.[0]?.['StbModel']?.[0]?.['StbMembers']?.[0]?.['StbColumns']?.[0]?.['StbColumn'];
  if (!columns) return;
  ...
  columns.forEach((column) => {
    ...
    // Add condition_bottom and condition_top with default values
    if (!attrs['condition_bottom']) {
      attrs['condition_bottom'] = V202_DEFAULTS.StbColumn.condition_bottom;
      count++;
    }
    if (!attrs['condition_top']) {
      attrs['condition_top'] = V202_DEFAULTS.StbColumn.condition_top;
      count++;
    }
  });

  if (count > 0) {
    logger.warn(`StbColumn: Added ${count} estimated attributes (condition_bottom/top = PIN)`);
  }
}

/**
 * Convert StbSlab attributes from v2.0.2 to v2.1.0
 * @param {object} stbRoot - ST-Bridge root element
 */
export function convertSlabAttributesTo210(stbRoot) {
  const root = getStbRoot(stbRoot);
  const slabs = root?.[0]?.['StbModel']?.[0]?.['StbMembers']?.[0]?.['StbSlabs']?.[0]?.['StbSlab'];
  if (!slabs) return;
  ...
  slabs.forEach((slab) => {
    ...
    for (const [key, value] of Object.entries(V210_DEFAULTS.StbSlab)) {
      if (!attrs[key]) {
        attrs[key] = value;
        count++;
      }
  ...
  if (count > 0) {
    logger.info(`StbSlab: Added ${count} new attributes`);
  }
}

/**
 * Convert StbSlab attributes from v2.1.0 to v2.0.2
 * @param {object} stbRoot - ST-Bridge root element
 */
export function convertSlabAttributesTo202(stbRoot) {
  const root = getStbRoot(stbRoot);
  const slabs = root?.[0]?.['StbModel']?.[0]?.['StbMembers']?.[0]?.['StbSlabs']?.[0]?.['StbSlab'];
  if (!slabs) return;
  ...
  slabs.forEach((slab) => {
    ...
    // Remove v2.1.0 specific attributes
    const attrsToRemove = ['kind_structure', 'kind_slab', 'direction_load', 'isFoundation'];
  ...
  if (count > 0) {
    logger.info(`StbSlab: Removed ${count} v2.1.0 attributes`);
  }
}

/**
 * Attributes removed from S beam section elements in v2.1.0
 * (from StbSecSteelBeam_S_Straight, etc.)
 * Note: 'type' is REQUIRED in both 2.0.2 and 2.1.0, do NOT remove it
 */
const S_SECTION_ATTRS_REMOVED_IN_210 = [
  'joint_id_start',
  'joint_id_end',
  'strength_web',
  'strength_flange',
 ...
 */
const GUID_NOT_ALLOWED_ELEMENTS = [
  'StbParallelAxes',
  'StbStory',
  // Section elements
  'StbSecColumn_S',
...
/**
 * Attributes removed from RC section figure elements in v2.1.0
 * (from StbSecColumnRect, StbSecColumnCircle, etc.)
 */
const RC_SECTION_FIGURE_ATTRS_REMOVED_IN_210 = [
  'depth_cover_start_X',
  'depth_cover_end_X',
  'depth_cover_start_Y',
  'depth_cover_end_Y',
...
/**
 * Attributes removed from bar arrangement parent elements in v2.1.0
 * (from StbSecBarArrangementColumn_RC, StbSecBarArrangementBeam_RC, etc.)
 * These are moved to child elements (StbSecBarColumnRectSameSimple, etc.)
 */
const BAR_ARRANGEMENT_PARENT_ATTRS_REMOVED_IN_210 = [
  'depth_cover_start_X',
  'depth_cover_end_X',
  'depth_cover_start_Y',
  'depth_cover_end_Y',
      ...
      Object.keys(figure).forEach((key) => {
        ...
        if (Array.isArray(children)) {
          children.forEach((child) => {
            if (child['$']) {
              RC_SECTION_FIGURE_ATTRS_REMOVED_IN_210.forEach((attr) => {
...
export function removeBarArrangementParentAttrsTo210(stbRoot) {
  ...
  const processBarArrangement = (barArrangement) => {
    ...
    BAR_ARRANGEMENT_PARENT_ATTRS_REMOVED_IN_210.forEach((attr) => {
  ...
  const processSteelElements = (parentElement) => {
    ...
    Object.keys(parentElement).forEach((key) => {
      ...
      if (Array.isArray(children)) {
        children.forEach((child) => {
          if (child['$']) {
            S_SECTION_ATTRS_REMOVED_IN_210.forEach((attr) => {
              if (child['$'][attr] !== undefined) {
                delete child['$'][attr];
                count++;
              }
...
/**
 * Remove invalid attributes from StbJointArrangement (pos, joint, kind_joint)
 * These attributes were part of v2.0.2 but not in v2.1.0
 * @param {object} stbRoot - ST-Bridge root element
 */
export function removeJointArrangementAttrsTo210(stbRoot) {
  ...
  // v2.1.0 StbJointArrangement requires id_section and starting_point
  // The Type7 conversion creates v2.0.2 format which is incompatible
  // Since we can't properly convert without StbSecJoint sections, remove entirely
  if (members['StbJointArrangements']) {
    const count = members['StbJointArrangements'][0]?.['StbJointArrangement']?.length || 0;
    delete members['StbJointArrangements'];
    if (count > 0) {
      logger.info(
        `Removed StbJointArrangements (${count} elements) - incompatible with v2.1.0 format`,
      );
    }
  }
}
...
export function applyAttributeChangesTo210(stbRoot) {
  ...
  convertColumnAttributesTo210(stbRoot);
  convertGirderBeamAttributesTo210(stbRoot);
  convertBraceAttributesTo210(stbRoot);
  convertSlabAttributesTo210(stbRoot);
  ...
  // Additional fixes for v2.1.0 compliance
  removeRollCTypeAttributeTo210(stbRoot);
  removeJointArrangementAttrsTo210(stbRoot);
  ...
  // Structural conversion for taper elements
  ...
  // New fixes for remaining errors
  removeInvalidSecPipeTo210(stbRoot);
  renameSecBaseConventionalTo210(stbRoot);
  removePileStrengthConcreteTo210(stbRoot);
  removeBarArrangementPileTo210(stbRoot);
  removeFigurePileSTo210(stbRoot);
  removeEmptyBarArrangementFoundationTo210(stbRoot);
  removeInvalidSecPileProductTo210(stbRoot);
  // Latest fixes
  removeInvalidSecOpenTo210(stbRoot);
  removeStbExtensionsTo210(stbRoot);
  removeFigurePileRCTo210(stbRoot);
  removeEmptySecPileSTo210(stbRoot);
  // Final pile and joint fixes
  removeEmptySecPileRCTo210(stbRoot);
...
export function applyAttributeChangesTo202(stbRoot) {
  ...
  convertColumnAttributesTo202(stbRoot);
  convertSlabAttributesTo202(stbRoot);
}
