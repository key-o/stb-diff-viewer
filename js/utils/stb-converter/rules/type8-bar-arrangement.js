...
function convertBeamBarArrangement(barArrangement) {
  ...
  if (simpleElements) {
    simpleElements.forEach((simpleElement) => {
      ...
      // StbSecBarBeamSimple attributes (stirrup, cover, etc.)
      const simpleAttrs = {};

      // Stirrup attributes (stay on StbSecBarBeamSimple)
      if (oldAttrs['D_stirrup']) simpleAttrs['D_stirrup'] = oldAttrs['D_stirrup'];
      if (oldAttrs['D_bar_spacing']) simpleAttrs['D_bar_spacing'] = oldAttrs['D_bar_spacing'];
      if (oldAttrs['strength_stirrup'])
        simpleAttrs['strength_stirrup'] = oldAttrs['strength_stirrup'];
      if (oldAttrs['strength_bar_spacing'])
        simpleAttrs['strength_bar_spacing'] = oldAttrs['strength_bar_spacing'];
      ...
      simpleElement['$'] = simpleAttrs;
      simpleElement['StbSecBarBeamSimpleMain'] = mainElements;
    });
  }

  // Process StbSecBarBeamComplex - more complex structure, skip for now
  // These require multiple child elements with specific structure
