...
export class SuggestionEngine {
  ...
  generateSuggestions(elementType, attributeName, context) {
    ...
    const mergeEntries = (entries) => {
      entries.forEach((entry) => {
        ...
        const hasDescriptiveLabel = existing.label && existing.label !== existing.value;
        const incomingHasBetterLabel = normalized.label && normalized.label !== normalized.value;
    ...
    mergeEntries(this.getDynamicSuggestions(elementType, attributeName, context));

    // 3. 構造関連サジェスト
    mergeEntries(this.getStructuralSuggestions(elementType, attributeName, context));
   ...
   */
  getDynamicSuggestions(elementType, attributeName, _context) {
    const suggestions = [];
    const seen = new Set();
    ...
    try {
      ...
      log.warn(`動的サジェストの取得中にエラーが発生しました:`, error);
    }
   ...
   */
  getStructuralSuggestions(elementType, attributeName, _context) {
    const suggestions = [];
    ...
    try {
      ...
      log.warn(`構造関連サジェストの取得中にエラーが発生しました:`, error);
    }
    ...
    try {
      ...
      for (const doc of docs) {
        ...
        sectionElements.forEach((section) => {
          ...
          const shape = section.getAttribute('shape') || section.getAttribute('shape_name');
          const strength =
            section.getAttribute('strength_name') || section.getAttribute('material');
          ...
          const labelSuffix = detailParts.length ? ` – ${detailParts.join(', ')}` : '';
      ...
      log.warn('断面サジェストの取得中にエラーが発生しました:', error);
    }
    ...
    try {
      ...
      docs.forEach((doc) => {
        ...
        nodes.forEach((node) => {
          ...
          const coords = [x, y, z].filter((val) => val !== null && val !== undefined);
      ...
      log.warn('ノードサジェストの取得中にエラーが発生しました:', error);
    }
  ...
  /**
   * 使用統計をstorageHelperから読み込み
   */
  loadUsageStats() {
    const stored = storageHelper.get('usageStats');
    if (stored) {
      this.usageStats = new Map(Object.entries(stored));
    }
  }

  /**
   * 使用統計をstorageHelperに保存
   */
  saveUsageStats() {
    const data = Object.fromEntries(this.usageStats);
    storageHelper.set('usageStats', data);
  }
