 ...
 * @property {string} attribute - 属性名
 * @property {*} oldValue - 元の値
 ...
 * @property {number} totalRepairs - 総修復数
 * @property {number} successCount - 成功数
 * @property {number} failureCount - 失敗数
 * @property {RepairResult[]} results - 修復結果の詳細
 * @property {Date} timestamp - 実行日時
 */
...
export class StbRepairEngine {
  ...
  repairDataIssue(issue, useDefaults) {
    ...
    else if (issue.message.includes('数値ではありません') ||
             issue.message.includes('無限大')) {
      if (useDefaults && attribute in DEFAULT_VALUES) {
        const defaultValue = DEFAULT_VALUES[attribute];
  ...
  repairGeometryIssue(issue) {
    ...
    // 長さゼロの要素や非常に短い要素を削除
    if (issue.message.includes('同じノード') ||
        issue.message.includes('非常に短い')) {
      this.removeElement(element);
...
export function formatRepairReport(report) {
  ...
  lines.push('='.repeat(60));
  lines.push('');
  lines.push(`実行日時: ${report.timestamp.toISOString()}`);
  lines.push('');
  ...
  lines.push(`削除された要素数: ${report.removedElements.length}`);
  lines.push('');
